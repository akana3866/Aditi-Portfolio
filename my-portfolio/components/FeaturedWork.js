...
function ProjectCard({ title, tags, description, imageUrl }) {
    return (
      <div className="w-80 h-[488px] px-8 py-9 relative bg-white rounded-2xl shadow-sm flex flex-col justify-end gap-10 overflow-hidden border border-gray-100">
        <div className="flex flex-col gap-5 z-10">
          ...
          <div className="flex gap-3 flex-wrap">
            {tags.map((tag, i) => (
              <span 
                key={i} 
                className="px-2.5 py-1.5 rounded-md border border-gray-400 text-gray-600 text-xs"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="px-12 py-9 flex flex-col gap-8">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-1.5">
            <h2 className="text-black text-3xl font-medium">Featured Work</h2>
